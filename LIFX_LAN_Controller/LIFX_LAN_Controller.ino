...
LifxDevice lifxDevices[MAX_DEVICES];
int deviceCount = 0;
...
void discoverLifxDevices() {
  ...
  while (millis() - startTime < 5000) { // Listen for 5 seconds
    if (lifxUdp.parsePacket()) {
      ...
      if (packetSize >= sizeof(LifxPacket)) {
        ...
        if (packet->type == LIFX_STATE_SERVICE) {
          responses++;
          Serial.println("  *** LIFX DEVICE FOUND! ***");
        }
      } else {
        Serial.println("  Not a LIFX packet (too small)");
      }
      
      // Print first few bytes for debugging
      Serial.print("  Data: ");
      for (int i = 0; i < min(16, packetSize); i++) {
  ...
  Serial.print(" devices from ");
  Serial.print(totalPackets);
  Serial.println(" total packets received.");
  
  if (totalPackets == 0) {
    Serial.println("No packets received. Possible issues:");
    Serial.println("1. No LIFX bulbs on this network");
    Serial.println("2. Router blocks UDP broadcast packets");
    Serial.println("3. LIFX bulbs are powered off");
    Serial.println("4. LIFX bulbs are on different network");
  }
...
void handleStateService(uint8_t* payload, size_t payloadSize, IPAddress remoteIP) {
  if (payloadSize >= 5) {
    ...
    if (deviceCount < MAX_DEVICES) {
      lifxDevices[deviceCount].ip = remoteIP;
      lifxDevices[deviceCount].connected = true;
      lifxDevices[deviceCount].lastSeen = millis();
      lifxDevices[deviceCount].label = "Unknown";
      lifxDevices[deviceCount].power = false;
      lifxDevices[deviceCount].hue = 0;
      lifxDevices[deviceCount].saturation = 0;
      lifxDevices[deviceCount].brightness = 0;
      lifxDevices[deviceCount].kelvin = 3500;
      
      // Extract MAC from packet target field
      // Note: This is simplified - in practice you'd need to match by IP
      memset(lifxDevices[deviceCount].mac, 0, 6);
...
void handleStateLabel(uint8_t* payload, size_t payloadSize, IPAddress remoteIP) {
  if (payloadSize >= 32) {
    ...
    for (int i = 0; i < deviceCount; i++) {
      if (lifxDevices[i].ip == remoteIP) {
        lifxDevices[i].label = String(label);
        Serial.print("Device ");
        Serial.print(i);
        Serial.print(" label: ");
        Serial.println(lifxDevices[i].label);
        break;
      }
    }
  }
...
void handleStateLight(uint8_t* payload, size_t payloadSize, IPAddress remoteIP) {
  if (payloadSize >= 13) {
    ...
    for (int i = 0; i < deviceCount; i++) {
      if (lifxDevices[i].ip == remoteIP) {
        lifxDevices[i].hue = hue;
        lifxDevices[i].saturation = saturation;
        lifxDevices[i].brightness = brightness;
        lifxDevices[i].kelvin = kelvin;
        lifxDevices[i].power = (power > 0);
        lifxDevices[i].lastSeen = millis();
        ...
        Serial.print(kelvin);
        Serial.print(" Power:");
        Serial.println(lifxDevices[i].power ? "ON" : "OFF");
        break;
      }
    }
  }
...
void handleSerialCommand(String command) {
  ...
  if (command == "discover") {
    ...
    if (deviceIndex >= 0 && deviceIndex < deviceCount) {
      ...
      sendEchoRequest(&lifxDevices[deviceIndex]);
    } else {
    ...
    if (firstSpace > 0) {
      ...
      if (deviceIndex >= 0 && deviceIndex < deviceCount) {
        ...
        Serial.println(power ? "ON" : "OFF");
        setDevicePower(&lifxDevices[deviceIndex], power);
      } else {
        Serial.println("Invalid device index");
      }
    } else {
    ...
    if (firstSpace > 0) {
      ...
      if (space1 > 0 && space2 > 0 && space3 > 0) {
        ...
        if (deviceIndex >= 0 && deviceIndex < deviceCount) {
          ...
          setDeviceColor(&lifxDevices[deviceIndex], hue, sat, bright);
        } else {
          Serial.println("Invalid device index");
        }
      } else {
    ...
    if (firstSpace > 0) {
      ...
      if (deviceIndex >= 0 && deviceIndex < deviceCount) {
        ...
        Serial.println(bright);
        setDeviceColor(&lifxDevices[deviceIndex], 0, 0, bright, 3500);
      } else {
        Serial.println("Invalid device index");
      }
    } else {
...
void checkDeviceStatus() {
  ...
  for (int i = 0; i < deviceCount; i++) {
    if (millis() - lifxDevices[i].lastSeen > 60000) { // 1 minute timeout
      lifxDevices[i].connected = false;
      Serial.print("Device ");
      Serial.print(i);
